...
<tmpl:override name="rightBox">
    ...
    <div class="button-wrapper">
        ...
        <div class="right">
            <form class="form-inline" role="form" action="${pageContext.request.contextPath}/weixin/user/list">
                ...
                <div class="form-group">
                    <label>微信分组</label>
                    <select name="groupWechatId">
                        <option value="">请选择</option>
                        <c:forEach var="groupWechat" items="${groupWechatEntities}">
                            <option value="${groupWechat.id}"
                                    <c:if test="${groupWechat.id eq user.groupWechatId}">
                                        selected</c:if>>${groupWechat.name}</option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>虚拟分组</label>
                    <select name="groupVirtualId">
                        <option value="">请选择</option>
                        <c:forEach var="groupVirtual" items="${groupVirtualEntities}">
                            <option value="${groupVirtual.id}"
                                    <c:if test="${groupVirtual.id eq user.groupVirtualId}">
                                        selected</c:if>>${groupVirtual.name}</option>
                        </c:forEach>
                    </select>
                </div>
    ...
    <jsp:include page="../../shared/pagination.jsp" flush="true">
        ...
        <jsp:param name="paginationURL" value="${pageContext.request.contextPath}/weixin/user/list?nickname=${user.nickname}&groupWechatId=${user.groupWechatId}&groupVirtualId=${user.groupVirtualId}"/>
    </jsp:include>
</tmpl:override>
